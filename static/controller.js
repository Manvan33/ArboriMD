...
function create_folders(data) {
    Object.keys(data).forEach(name => {
        create_folder(name, data[name].parent, data[name].notes);
    });
}
function create_folder(name, parent, notes) {
    if (name.includes("/")) {
...
function open_note(note) {
    note.select();
    let url = CODIMD_URL + note.id + "?edit";
    document.querySelector('#codimd').src = url;
    console.log("Opening note " + id);
    refresh_note(note.id);
}

function refresh_note(id) {
